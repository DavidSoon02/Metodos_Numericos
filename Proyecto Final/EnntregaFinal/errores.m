...
if isempty(valor_exacto) || isempty(valor_aproximado)
    ...
    h_fig = figure;
    text(0.5, 0.9, get(text_valor_absoluto, 'String'), 'FontSize', 14, 'HorizontalAlignment', 'center');
    text(0.5, 0.7, get(text_valor_relativo, 'String'), 'FontSize', 14, 'HorizontalAlignment', 'center');
    title('Resultados del cálculo de errores');
    axis off;

    % Guardar la figura como un archivo PDF
    nombre_archivo_pdf = 'resultados_calculo_errores.pdf';
