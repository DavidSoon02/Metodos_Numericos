...
if isempty(numero)
    ...
    if esBinario
        ...
        title('Resultados del cálculo de binario a decimal');
        axis off;

        % Guardar la figura como un archivo PDF
        nombre_archivo_pdf = 'resultados_sistemas_numeracion1.pdf';
...
if isempty(numero)
    ...
    if isnumeric(str2num(numero)) && isscalar(str2num(numero)) && str2num(numero) > 0
        ...
        h_fig = figure;
        text(0.5, 0.9, get(text_resultado, 'String'), 'FontSize', 14, 'HorizontalAlignment', 'center');
        title('Resultados del cálculo de decimal a binario');
        axis off;

        % Guardar la figura como un archivo PDF
        nombre_archivo_pdf = 'resultados_sistemas_numeracion2.pdf';
        print(h_fig, nombre_archivo_pdf, '-dpdf', '-bestfit');
    else
